...
interface UnifiedYearlyPaymentPlanProps {
...
const UnifiedYearlyPaymentPlan: React.FC<UnifiedYearlyPaymentPlanProps> = ({
  instantPowerAmount,
  superchargeDetails,
  otherUserAmounts = [],
  selectedDiscounts = [],
  ...
  const [numberOfPeriods, setNumberOfPeriods] = useState("12");
  ...
  // ✅ Slider value state (parity with mobile)
  ...
  // ✅ Fetch dynamic term bounds for yearly
  useEffect(() => {
    if (!amount || amount <= 0) {
      setMinTerm(1);
      setMaxTerm(12);
    ...
    termsMutation.mutate(
      ...
      },
      {
        onSuccess: (res: any) => {
          ...
          const newMax = Number(res?.data?.maxTerm ?? 12);
          setMinTerm(newMin);
          setMaxTerm(newMax);

          // Clamp slider and numberOfPeriods
          const clamped = Math.max(newMin, Math.min(sliderValue, newMax));
          if (clamped !== sliderValue) {
            setSliderValue(clamped);
            setNumberOfPeriods(String(clamped));
          }
        },
        onError: () => {
          setMinTerm(1);
  ...
  }, [amount, paymentFrequency, sliderValue, termsMutation]);

  // ✅ Sync slider with numberOfPeriods
  useEffect(() => {
    setNumberOfPeriods(String(sliderValue));
  }, [sliderValue]);
  ...
  const buildCommonFields = useCallback(
    (plan: any | null): CommonWithDiscounts => {
      ...
      const common: CommonWithDiscounts = {
        ...
        instantAmount: 0, // ✅ Yearly mode: instant is 0
        yearlyAmount: Number(amount.toFixed(2)), // ✅ Yearly mode: use yearlyAmount
    ...
    <>
      <div className="flex flex-col p-4 bg-white min-h-screen">
        <h1 className="text-2xl font-bold mb-6">
          {`${displayName ? `${displayName} — ` : ""}Yearly Plan $${displayTotal.toFixed(2)}`}
        </h1>
        ...
        {/* ✅ Slider Control (parity with mobile) */}
        <div className="mb-6">
          <div className="flex justify-between items-center mb-2">
            <label className="text-sm font-medium">
              Number of months {termsPending && <span className="text-xs text-gray-500">(updating…)</span>}
            </label>
            <span className="text-lg font-bold">{sliderValue} months</span>
          </div>
          ...
          {/* ✅ Per-period payment display */}
          <div className="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
            <p className="text-sm text-gray-600">
              Estimated payment:{" "}
              <span className="font-bold text-black">${perPeriodPayment.toFixed(2)}/month</span>
            </p>
          </div>
        </div>
                  ...
                  {(methods ?? [])
                    ...
                    .map((method: PaymentMethod) => (
                      <PaymentMethodItem
                        key={method.id}
                        method={method}
                        selectedMethod={selectedPaymentMethod} // ✅ fixed: no fake {} as PaymentMethod
                        onSelect={(m) => {
